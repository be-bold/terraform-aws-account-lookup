locals {
  ...
  search_step5_include_name = !local.is_include_name_set ? local.search_step4_include_state : local.include_name_matcher
  ...
  include_name_startswith = !local.is_include_name_set ? [] : [ for account in local.search_step4_include_state : account if anytrue([ for prefix in var.include.name.values : startswith(account.name, prefix) ]) ]
  include_name_endswith = !local.is_include_name_set ? [] : [ for account in local.search_step4_include_state : account if anytrue([ for suffix in var.include.name.values : endswith(account.name, suffix) ]) ]
  include_name_equals = !local.is_include_name_set ? [] : [ for account in local.search_step4_include_state : account if anytrue([ for value in var.include.name.values : account.name == value ]) ]
  include_name_contains = !local.is_include_name_set ? [] : [ for account in local.search_step4_include_state : account if anytrue([ for value in var.include.name.values : strcontains(account.name, value)]) ]
  include_name_regex = !local.is_include_name_set ? [] : [ for account in local.search_step4_include_state : account if anytrue([ for value in var.include.name.values : length(regexall(value, account.name)) > 0 ]) ]


  # STEP 6 - include joined method
  is_include_joined_method_set = var.include != null && var.include.joined != null && var.include.joined.method != null && try(length(var.include.joined.method) > 0, false)
  search_step6_include_joined_method = !local.is_include_joined_method_set ? local.search_step5_include_name : [ for account in local.search_step5_include_name : account if anytrue([ for value in var.include.joined.method : account.joined.method == value])  ]


  ## STEP 7 - include joined timestamp
  is_include_joined_timestamp_set = var.include != null && var.include.joined != null && var.include.joined.timestamp != null
  search_step7_exclude_joined_timestamp = !local.is_include_joined_timestamp_set ? local.search_step6_include_joined_method : local.include_joined_timestamp_is
  ...
  include_joined_timestamp_before = !local.is_include_joined_timestamp_set ? [] : [ for account in local.search_step6_include_joined_method : account if timecmp(account.joined.timestamp, var.include.joined.timestamp.other_timestamp) == -1 ]
  include_joined_timestamp_equals = !local.is_include_joined_timestamp_set ? [] : [ for account in local.search_step6_include_joined_method : account if timecmp(account.joined.timestamp, var.include.joined.timestamp.other_timestamp) == 0 ]
  include_joined_timestamp_after = !local.is_include_joined_timestamp_set ? [] : [ for account in local.search_step6_include_joined_method : account if timecmp(account.joined.timestamp, var.include.joined.timestamp.other_timestamp) == 1 ]


  ## STEP 8 - include tags
  is_include_tags_set = var.include != null && var.include.tags != null && try(length(var.include.tags) > 0, false)
  search_step8_include_tags = !local.is_include_tags_set ? local.search_step7_exclude_joined_timestamp : local.include_tags_result

  include_tags_accounts_containing_all_tags_from_filter_option = !local.is_include_tags_set ? [] : [ for account in local.search_step7_exclude_joined_timestamp : account if alltrue([ for tag_name, search_strings in var.include.tags : lookup(account.tags, tag_name, null) != null ]) ]
  ...
  ## STEP 13 - exclude name
  is_exclude_name_set = var.exclude != null && var.exclude.name != null
  search_step13_exclude_name = !local.is_exclude_name_set ? local.search_step12_exclude_state : local.exclude_name_matcher
  ...
  exclude_name_startswith = !local.is_exclude_name_set ? [] : [ for account in local.search_step12_exclude_state : account if alltrue([ for prefix in var.exclude.name.values : !startswith(account.name, prefix) ]) ]
  exclude_name_endswith = !local.is_exclude_name_set ? [] : [ for account in local.search_step12_exclude_state : account if alltrue([ for suffix in var.exclude.name.values : !endswith(account.name, suffix) ]) ]
  exclude_name_equals = !local.is_exclude_name_set ? [] : [ for account in local.search_step12_exclude_state : account if alltrue([ for value in var.exclude.name.values : account.name != value ]) ]
  exclude_name_contains = !local.is_exclude_name_set ? [] : [ for account in local.search_step12_exclude_state : account if alltrue([ for value in var.exclude.name.values : !strcontains(account.name, value) ]) ]
  exclude_name_regex = !local.is_exclude_name_set ? [] : [ for account in local.search_step12_exclude_state : account if alltrue([ for value in var.exclude.name.values : length(regexall(value, account.name)) == 0 ]) ]
  ...
  ## STEP 15 - exclude joined timestamp
  is_exclude_joined_timestamp_set = var.exclude != null && var.exclude.joined != null && var.exclude.joined.timestamp != null
  search_step15_exclude_joined_timestamp = !local.is_exclude_joined_timestamp_set ? local.search_step14_exclude_joined_method : local.exclude_joined_timestamp_is
  ...
  exclude_joined_timestamp_before = !local.is_exclude_joined_timestamp_set ? [] : [ for account in local.search_step14_exclude_joined_method : account if timecmp(account.joined.timestamp, var.exclude.joined.timestamp.other_timestamp) >= 0 ]
  exclude_joined_timestamp_equals = !local.is_exclude_joined_timestamp_set ? [] : [ for account in local.search_step14_exclude_joined_method : account if timecmp(account.joined.timestamp, var.exclude.joined.timestamp.other_timestamp) != 0 ]
  exclude_joined_timestamp_after = !local.is_exclude_joined_timestamp_set ? [] : [ for account in local.search_step14_exclude_joined_method : account if timecmp(account.joined.timestamp, var.exclude.joined.timestamp.other_timestamp) < 1 ]


  ## STEP 16 - exclude tags
  is_exclude_tags_set = var.exclude != null && var.exclude.tags != null && try(length(var.exclude.tags) > 0, false)
  search_step16_exclude_tags = !local.is_exclude_tags_set ? local.search_step15_exclude_joined_timestamp : local.exclude_tags_result

  exclude_tags_accounts_matching_criteria_for_exclusion = !local.is_exclude_tags_set ? [] : [ for account in local.search_step15_exclude_joined_timestamp : account.id if alltrue([ for tag_name, search_strings in var.exclude.tags : lookup(account.tags, tag_name, null) != null ]) && alltrue([ for tag_name, search_strings in var.exclude.tags : anytrue([ for search_string in search_strings : lookup(account.tags, tag_name, null) == search_string ]) ]) ]
  exclude_tags_result = !local.is_exclude_tags_set ? [] : [ for account in local.search_step15_exclude_joined_timestamp : account if !contains(local.exclude_tags_accounts_matching_criteria_for_exclusion, account.id) ]
