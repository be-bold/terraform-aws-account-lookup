run "filter_using_exclude.email_-_successfully_filter_for_single_entry_with_assertions_on_all_properties" {
  variables {
    input = [
      {
        id     = "123456789012"
...
run "filter_using_exclude.email_-_successfully_filter_for_multiple_entries" {
  variables {
    input = [
      {
        id     = "123456789012"
...
run "filter_using_exclude.email_-_only_return_known_entry_if_you_filter_for_known_and_unknown_entry" {
  variables {
    input = [
      {
        id     = "123456789012"
  ...
  assert {
    condition = length(keys(local.search_result)) == 2
    error_message = "Expected 1 entries in search result."
  }
...
run "filter_using_exclude.email_-_unknown_entry_doesn't_exclude_anything" {
  variables {
    input = [
      {
        id     = "123456789012"
