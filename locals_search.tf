locals {
  ...
  search_step1_include_id = !local.is_include_id_set ? local.search_base : [ for account in local.search_base : account if anytrue([ for value in var.include.id : account.id == value])  ]
  ...
  exclude_tags_result = !local.is_exclude_tags_set ? [] : [ for account in local.exclude_tags_accounts_containing_all_tags_from_filter_option : account if alltrue([ for tag_name, search_strings in var.exclude.tags : alltrue([ for search_string in search_strings : account.tags[tag_name] != search_string ]) ]) ]
